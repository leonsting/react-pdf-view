...
export class PDFReader extends React.Component<IProps, IStates> {
    ...
    public componentDidMount () {
      ...
      if (url) {
        ...
        pdfjsLib.getDocument(obj).promise.then((pdf) => {
    ...
    private renderPage(dom, spnum) {
      ...
      pdf.getPage(currentPage).then((page) => {
        ...
        if (scale) {
          ...
          let templeView = page.getViewport({ scale : DEFAULT_SCALE});
          desireScale = desiredWidth / templeView.width;
        }
        const viewport = page.getViewport({ scale : desireScale});
        const canvas = dom;
           ...
           <div id="my-pdf" style={style} className={styles["pdf__container"]}>
             {
               showAllPage ? <React.Fragment>
                              ...
                              {tempArr.length > 0 && addCustomPage && addCustomPage(this['canvas' + tempArr.length])}
                          </React.Fragment>
                          :
